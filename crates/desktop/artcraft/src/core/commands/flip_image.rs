...
pub fn flip_image(image: &str) -> Result<String, String> {
  ...
  let image = ImageReader::new(Cursor::new(bytes)).with_guessed_format().map_err(|err| format!("Image format error: {}", err))?.decode().map_err(|err| format!("Image decode error: {}", err))?;
  ...
  rotated_image.write_with_encoder(encoder).map_err(|err| format!("Image encoding error: {}", err))?;

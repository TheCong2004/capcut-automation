...
async fn enqueue(task_database: &TaskDatabase, prompt: &str) -> AnyhowResult<String> {
  ...
  let job_id = create_pipeline_job(CreatePipelineJobArgs { db: task_database.get_connection(), status: TaskStatus::Pending, current_stage: PipelineStage::Queued, maybe_input_payload: Some(&input_payload) }).await?;

...
pub async fn download_subtitle_content(
    ...
    let temp_dir = tempfile::tempdir()
        .map_err(|error| format!("Failed to create subtitle temporary directory: {error}"))?;
    let output_template = temp_dir.path().join("subtitle.%(ext)s");
    let output_template = output_template.to_string_lossy().to_string();
    let subtitle_flag = if is_auto {
        "--write-auto-subs"
    } else {
        "--write-subs"
    };
    let requested_format = if format.trim().is_empty() {
        "srt"
    } else {
        format.as_str()
    };
    ...
    .await?;
    if !output.success {
        return Err(parse_ytdlp_error(&output.stderr)
            .map(|error| error.to_wire_string())
            .unwrap_or_else(|| output.stderr.trim().to_string()));
    }

    let subtitle_path = std::fs::read_dir(temp_dir.path())
        .map_err(|error| format!("Failed to inspect downloaded subtitle: {error}"))?
        .filter_map(Result::ok)
        .map(|entry| entry.path())
        .find(|path| path.is_file())
        .ok_or_else(|| "yt-dlp did not produce a subtitle file".to_string())?;
    std::fs::read_to_string(&subtitle_path)
        .map_err(|error| format!("Failed to read downloaded subtitle: {error}"))
}
